...
We find the surprising result that mean weight observations seem to matter little in decision-making for two very different utility functions ($U_{yield}$ and $U_{HARA}$).
...
While the trophy fishing utility function we used has a very non-linear profile (being a step function), our results indicate that for spasmodically recruiting fish stocks, policies informed by mean fish weight can be expected to outperform stock-biomass-only policies when the utility function has a non-trivial fish weight dependence.
